    ...
    E: Engine,
{
    pub fn create(d: Device, priority: bool) -> GPUResult<SingleMultiexpKernel<E>> {
        info!("Begin Compile source file");
        let now = Instant::now();
        let src = sources::kernel::<E>(true);
        let pq = ProQue::builder().device(d).src(src).dims(1).build()?;
        println!("End Compile source file, spend{} ms", now.elapsed().as_millis());
        drop(now);
    ...
    E: Engine,
{
    ...
    where
        G: CurveAffine,
        <G as pairing::CurveAffine>::Engine: pairing::Engine,
    {
                    ...
                    .zip(self.kernels.iter_mut())
                {
                    threads.push(s.spawn(
                        move |_| -> Result<<G as CurveAffine>::Projective, GPUError> {
                            let mut acc = <G as CurveAffine>::Projective::zero();
                            for (bases, exps) in bases.chunks(kern.n).zip(exps.chunks(kern.n)) {  // multi thread
